...
/* 컬럼명이 하나의 테이블에만 있을 때는 테이블명 생략 가능
   같은 컬럼명이 여러 티에블에 있다면 테이블명 작성 */

/* 두 테이블이 결합하면 조회된 레코드 수는 A테이블 row * B테이블 row
   컬럼 수는 A테이블 열 + B테이블 열
   */
...
/* 1996년 7월 4일에 주문한 고객명 (customers,orders)
   1996년 7월 4일에 주문을 담당한 직원명 (employees,orders)*/
...
SELECT p.ProductName, s.SupplierName, c.CategoryName, s.Phone
FROM products p
JOIN suppliers s
...
/* 'Chang' 상품의 공급자명, 카테고리명 조회
   1996년 7월 9일에 주문한 고객명과, 처리한 직원명
   1996년 7월 9일에 주문한 상품명 ( orders, orderDetails, products)
   */

SELECT *
FROM products p JOIN suppliers s ON p.SupplierID
    JOIN categories c
